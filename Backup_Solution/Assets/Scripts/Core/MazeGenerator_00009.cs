...
namespace Code.Lavos.Core
{
    ...
    [SerializeField] private bool useSeedProgression = true;  // Auto-increment seed per level
    ...
    {
        // Use SeedProgression if enabled
        if (useSeedProgression && SeedProgression.Instance != null)
        {
            seedString = SeedProgression.Instance.CurrentSeed;
            Debug.Log($"[MazeGenerator] Using SeedProgression - Level {SeedProgression.Instance.CurrentLevel}: {seedString}");
        }
    ...
    private int CalculateComplexity()
    {
        ...
        int seedFactor = string.IsNullOrEmpty(seedString) ? 1 : seedString.Length * 10;
        return (sizeFactor + seedFactor) / 100;
    }

    /// <summary>
    /// Generate maze and advance seed progression.
    /// Call this when loading a new level/scene.
    /// </summary>
    public void GenerateNextLevel()
    {
        if (SeedProgression.Instance != null)
        {
    ...
    /// <summary>
    /// Generate maze with numeric seed (backward compatible).
    /// </summary>
    public void Generate(uint numericSeed)
    {
        _seed = numericSeed;
    ...
    {
        return string.IsNullOrEmpty(seedString) ? _seed.ToString() : seedString;
    }
