...
namespace Code.Lavos.Core
{
    ...
    public class MazeRenderer : MonoBehaviour
    {
        ...
        /// <summary>
        /// Place interior walls on ALL cell boundaries (Room/Corridor vs Wall).
        /// Supports 8-way walls (N, NE, E, SE, S, SW, W, NW).
        /// Walls are snapped to cell edges and CONNECT properly.
        /// </summary>
        private void PlaceInteriorWalls(Transform parent, ref int spawned)
        {
            int wallPlacedCount = 0;
            HashSet<string> placedWalls = new HashSet<string>(); // Prevent duplicates

            int walkableCount = 0, wallCellCount = 0;
            ...
            for (int x = 0; x < mazeSize; x++)
            {
                for (int y = 0; y < mazeSize; y++)
                {
                    ...
                    if (current == GridMazeCell.Room || current == GridMazeCell.Corridor)
                        walkableCount++;
                    else if (current == GridMazeCell.Wall)
                        wallCellCount++;

                    // Check boundaries from walkable cells (Room/Corridor)
                    bool currentWalkable = (current == GridMazeCell.Room || current == GridMazeCell.Corridor);

                    // Check EAST boundary (90°) - vertical wall at x+1
                    if (x + 1 < mazeSize)
                    {
                        GridMazeCell east = grid.GetCell(x + 1, y);
                        bool eastWalkable = (east == GridMazeCell.Room || east == GridMazeCell.Corridor);

                        if (currentWalkable != eastWalkable)
                        {
                            string wallKey = $"V_{x + 1}_{y}"; // Vertical wall key
                            if (!placedWalls.Contains(wallKey))
                            {
                                Vector3 pos = new Vector3(
                                    (x + 1) * cellSize,
                                    wallHeight / 2f,
                    ...
                    // Check SOUTH boundary (180°) - horizontal wall at y+1
                    if (y + 1 < mazeSize)
                    {
                        GridMazeCell south = grid.GetCell(x, y + 1);
                        bool southWalkable = (south == GridMazeCell.Room || south == GridMazeCell.Corridor);

                        if (currentWalkable != southWalkable)
                        {
                            string wallKey = $"H_{x}_{y + 1}"; // Horizontal wall key
                            if (!placedWalls.Contains(wallKey))
                            {
                                Vector3 pos = new Vector3(
                                    x * cellSize + cellSize / 2f,
            ...
            Debug.Log($"[MazeRenderer] Grid stats - Walkable: {walkableCount}, Wall cells: {wallCellCount}");
            Debug.Log($"[MazeRenderer]  Interior walls: {wallPlacedCount} placed (unique, connected)");
        }
