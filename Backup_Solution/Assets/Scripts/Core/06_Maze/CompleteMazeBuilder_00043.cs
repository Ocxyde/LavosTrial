...
namespace Code.Lavos.Core
{
    ...
    public class CompleteMazeBuilder : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // Load saved seed string first (for saved game loading)
            string savedSeedString = MazeSaveData.LoadSeedString();
            if (!string.IsNullOrEmpty(savedSeedString))
            {
                // Restore seed from saved data
                currentSeed = savedSeedString;
                seed = ComputeSeed(currentSeed);
                Log($"[CompleteMazeBuilder] Seed restored from save: {currentSeed}");
            }
            else
            {
                // Get seed from SeedManager if available (new game or progressive)
                var seedManager = FindObjectOfType<SeedManager>();
                if (seedManager != null && !string.IsNullOrEmpty(seedManager.CurrentSeed))
                {
                    currentSeed = seedManager.CurrentSeed;
                    Log($"[CompleteMazeBuilder] Seed loaded from SeedManager: {currentSeed}");
                }
                else
                {
                    seed = ComputeSeed(currentSeed);
        ...
        private void SaveMaze()
        {
            ...
            // Save to legacy MazeSaveData (PlayerPrefs) with seed string
            MazeSaveData.SaveGridMaze((int)seed, grid.SerializeToBytes(), spawnCell.x, spawnCell.y, currentSeed);
