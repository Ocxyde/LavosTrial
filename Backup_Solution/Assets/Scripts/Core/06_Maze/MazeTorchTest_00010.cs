...
namespace Code.Lavos.Core
{
    ...
    public class MazeTorchTest : MonoBehaviour
    {
        ...
        private void SpawnTestPlayer()
        {
            ...
            // Spawn player in the MIDDLE of the maze
            // Maze cells are centered at (x + 0.5) * cellSize
            // For 21x21 maze, middle cell is (10, 10)
            // Center position = (10.5 * 6, 1, 10.5 * 6) = (63, 1, 63)
            float cellSize = 6f;
            int middleCell = mazeWidth / 2; // Cell 10 for 21-wide maze
            float spawnX = (middleCell + 0.5f) * cellSize; // CENTER of cell
            float spawnZ = (middleCell + 0.5f) * cellSize; // CENTER of cell
            float spawnY = 1f; // On ground plane (floor is at y=0)
            
            _testPlayer.transform.position = new Vector3(spawnX, spawnY, spawnZ);

            Log($"[MazeTorchTest] Spawning at cell ({middleCell},{middleCell}) = World ({spawnX}, {spawnY}, {spawnZ})");
            ...
            Log($"[MazeTorchTest] Player spawned at {_testPlayer.transform.position} (CENTER of maze cell)");
