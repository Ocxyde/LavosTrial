...
namespace Code.Lavos.Core
{
    ...
    public class GridMazeGenerator
    {
        ...
        /// Spawn room is 3x3 with player spawning in CENTER (not corner).
        /// LEFT WALL is fully OPEN (3 cells) for entrance corridor.
        /// </summary>
        private void PlaceSpawnRoom()
        {
            ...
            int openWall = cfg.spawnRoomOpenWall; // Default 0 (left)
            ...
            // Mark spawn room area (LEFT WALL open for entrance)
            for (int dx = 0; dx < spawnRoomSize; dx++)
            {
                for (int dy = 0; dy < spawnRoomSize; dy++)
                {
                    ...
                    if (gridX >= 0 && gridX < gridSize && gridY >= 0 && gridY < gridSize)
                    {
                        ...
                        // OPEN WALL (left side by default) is Floor for entrance
                        else if (openWall == 0 && dx == 0)
                        {
            ...
            spawnEntranceDirection = Vector2Int.left;   // Full left wall open
            spawnExitDirection = Vector2Int.right;       // Full right wall for exit corridor
            ...
            Debug.Log($"[GridMazeGenerator]  Entrance: FULL LEFT WALL OPEN ({spawnRoomSize} cells)");
            Debug.Log($"[GridMazeGenerator]  Exit: FULL RIGHT WALL for corridor");
        }
