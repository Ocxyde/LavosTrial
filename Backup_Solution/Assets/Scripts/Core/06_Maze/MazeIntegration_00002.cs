...
namespace Code.Lavos.Core
{
    ...
    public class MazeIntegration : MonoBehaviour
    {
        ...
        [SerializeField] private TorchPlacer torchPlacer;  // Optional torch placer
        ...
        private void FindComponents()
        {
            ...
            if (torchPlacer == null)
                torchPlacer = GetComponent<TorchPlacer>();  // Auto-find if not assigned
        }
        ...
        public void GenerateMaze()
        {
            ...
            // Step 6: Place torches (optional, independent from maze generation)
            if (placeTorches && torchPlacer != null)
            {
                Debug.Log("[MazeIntegration] Step 6: Placing torches...");
                torchPlacer.PlaceTorches();
            }
            else if (placeTorches && torchPlacer == null)
            {
                Debug.LogWarning("[MazeIntegration] placeTorches is true but TorchPlacer component not found!");
            }
            ...
            if (torchPlacer != null && placeTorches)
                Debug.Log($"Torches Placed: {torchPlacer.TorchCount}");
        }
