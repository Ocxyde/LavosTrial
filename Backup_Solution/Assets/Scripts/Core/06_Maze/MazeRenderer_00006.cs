...
namespace Code.Lavos.Core
{
    ...
    private void CreateMaterials()
    {
        ...
        // PLUG-IN-AND-OUT: Request floor material via EventHandler
        if (EventHandler.Instance != null)
        {
            _floorMat = EventHandler.Instance.RequestMaterial(floorType, ProceduralCompute.TextureType.Floor);
            Debug.Log($"[MazeRenderer] Requested floor material via EventHandler: {floorType}");
        }
        else
        {
            // Fallback if EventHandler not available
            _floorMat = FloorMaterialFactory.GetFloorMaterial(floorType);
            Debug.LogWarning("[MazeRenderer] EventHandler not found, using FloorMaterialFactory directly");
        }
