...
namespace Code.Lavos.Core
{
    ...
    public class CompleteMazeBuilder : MonoBehaviour
    {
        ...
        {
            Log($"[CompleteMazeBuilder]  Placing walls at cell borders...");
            ...
            for (int x = 0; x < mazeSize; x++)
            {
                for (int y = 0; y < mazeSize; y++)
                {
                    ...
                    {
                        // Check adjacent cells to determine wall orientation and position
                        bool wallNorth = (y + 1 < mazeSize && grid.GetCell(x, y + 1) == GridMazeCell.Wall);
                        bool wallSouth = (y - 1 >= 0 && grid.GetCell(x, y - 1) == GridMazeCell.Wall);
                        bool wallEast = (x + 1 < mazeSize && grid.GetCell(x + 1, y) == GridMazeCell.Wall);
                        bool wallWest = (x - 1 >= 0 && grid.GetCell(x - 1, y) == GridMazeCell.Wall);

                        // Determine if wall runs North-South (vertical) or East-West (horizontal)
                        bool isVertical = wallNorth || wallSouth;
                        bool isHorizontal = wallEast || wallWest;

                        // Calculate position at cell BORDER (not center)
                        Vector3 pos;
                        Quaternion rot;

                        if (isVertical && !isHorizontal)
                        {
                            // Vertical wall - position at east border of cell
                            pos = new Vector3(
                                (x + 1) * cellSize,
                                wallHeight / 2f,
                        ...
                        {
                            // Horizontal wall - position at north border of cell
                            pos = new Vector3(
                        ...
                        {
                            // Corner or isolated - default to center with no rotation
                            pos = new Vector3(
                                x * cellSize + cellSize / 2f,
                                wallHeight / 2f,
                                y * cellSize + cellSize / 2f
            ...
            Log($"[CompleteMazeBuilder]  {spawned} walls placed at cell borders");
        }
