...
namespace Code.Lavos.Core
{
    ...
    public class GridMazeGenerator
    {
        ...
        private bool IsInSpawnCorridorArea(int x, int y)
        {
            ...
            // West corridor (from spawn room to west edge)
            // Only block cells WEST of spawn room, within corridor width
            if (x < spawnX && Mathf.Abs(y - spawnY) <= halfWidth)
            {
                return true;  // In west corridor
            }
            
            // East corridor (from spawn room to east edge)  
            // Only block cells EAST of spawn room, within corridor width
            if (x > spawnX && Mathf.Abs(y - spawnY) <= halfWidth)
            {
                return true;  // In east corridor
            }
