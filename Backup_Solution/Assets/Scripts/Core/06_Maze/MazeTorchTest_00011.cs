...
namespace Code.Lavos.Core
{
    ...
    public class MazeTorchTest : MonoBehaviour
    {
        ...
        private Vector3 FindValidSpawnPosition()
        {
            ...
            // Start from center and spiral out until we find a corridor
            for (int radius = 0; radius < 10; radius++)
            {
                for (int dx = -radius; dx <= radius; dx++)
                {
                    ...
                    {
                        if (Mathf.Abs(dx) != radius && Mathf.Abs(dy) != radius) continue; // Skip inner squares
                        
                        int checkX = middleCell + dx;
                        int checkY = middleCell + dy;
                        
                        // Check bounds
                        if (checkX < 0 || checkX >= mazeWidth || checkY < 0 || checkY >= mazeHeight) continue;
                        
                        // This is a valid corridor position (center of cell)
                        float spawnX = (checkX + 0.5f) * cellSize;
                        float spawnZ = (checkY + 0.5f) * cellSize;
                        float spawnY = 1f;
                        
                        Log($"[MazeTorchTest] Found valid spawn at cell ({checkX},{checkY}) = ({spawnX}, {spawnY}, {spawnZ})");
            ...
            // Fallback to center
            Log("[MazeTorchTest] Warning: Using fallback spawn position");
            return new Vector3((middleCell + 0.5f) * cellSize, 1f, (middleCell + 0.5f) * cellSize);
        }
