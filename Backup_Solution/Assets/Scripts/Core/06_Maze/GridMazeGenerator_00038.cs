...
namespace Code.Lavos.Core
{
    ...
    public class GridMazeGenerator
    {
        ...
        // Divisor for target cell calculation (carves ~33% of grid for better coverage)
        private const int TARGET_CELL_DIVISOR = 3;

        private void CarveMazeWithDfs()
        {
            // Use 4-way DFS for proper maze connectivity (walls between corridors)
            // 8-way creates diagonal corridors that break wall placement
            ...
            // Carve an initial corridor exit from spawn chamber (east side) to ensure connectivity
            int exitX = startX + 2; // East of spawn chamber
            int exitY = startY;
            if (IsValidMazeCell(exitX, exitY) && _grid[exitX, exitY] == GridMazeCell.Wall)
            {
                _grid[exitX, exitY] = GridMazeCell.Corridor;
                visited[exitX, exitY] = true;
                stack.Push(new Vector2Int(exitX, exitY));
            }
            ...
            Debug.Log($"[GridMazeGenerator] DFS target: {targetCells} cells ({TARGET_CELL_DIVISOR} divisor)");
        ...
        /// <summary>
        /// Expand corridor intersections to chambers while PRESERVING maze structure.
        /// Corridors connect chambers - chambers don't replace corridors!
        /// </summary>
        private void ExpandIntersectionsToChambers()
        {
            ...
            // Step 1: Find all corridor intersections (2+ neighbors = junction)
            List<Vector2Int> intersections = new List<Vector2Int>();
            for (int x = 3; x < _gridSize - 3; x++)
            {
                for (int y = 3; y < _gridSize - 3; y++)
                {
                    if (_grid[x, y] == GridMazeCell.Corridor && IsIntersection(x, y, false))
                    {
            ...
            // Step 3: Expand selected intersections to chambers
            foreach (Vector2Int intersection in intersections)
            {
                ...
                // 30% chance - keeps most corridors intact for proper maze feeling
                if (Random.value < 0.3f)
                {
                    int chamberSize = (Random.value < 0.7f) ? 3 : 5;

                    Debug.Log($"[GridMazeGenerator] Creating {chamberSize}x{chamberSize} chamber at {intersection}");

                    // Carve chamber - this converts corridor cells to room
            ...
            // Step 4: Ensure minimum chambers (fallback if no intersections qualified)
            if (chambersCreated < 3)
            {
                Debug.Log($"[GridMazeGenerator] Enforcing minimum chamber count (current: {chambersCreated})");
                
                // Try remaining intersections first
                foreach (Vector2Int intersection in intersections)
                {
                    if (chambersCreated >= 3) break;
                ...
                // Fallback: create chambers along corridor paths
                if (chambersCreated < 3)
                {
                    Debug.Log($"[GridMazeGenerator] No intersections - creating from corridor cells");
                    List<Vector2Int> corridorCells = new List<Vector2Int>();
                    for (int x = 5; x < _gridSize - 5; x++)
                    {
                        for (int y = 5; y < _gridSize - 5; y++)
                        {
                            if (_grid[x, y] == GridMazeCell.Corridor)
                            {
                    ...
                    // Shuffle
                    for (int i = corridorCells.Count - 1; i > 0; i--)
                    {
                        int j = Random.Range(0, i + 1);
                        Vector2Int temp = corridorCells[i];
                        corridorCells[i] = corridorCells[j];
                        corridorCells[j] = temp;
                    }
                    ...
                    foreach (Vector2Int cell in corridorCells)
                    {
                        ...
                        bool tooClose = false;
                        foreach (Vector2Int center in _chamberCenters)
                        {
                            if (Vector2Int.Distance(cell, center) < 6f)
                            {
                                tooClose = true;
                                break;
                            }
                        ...
                        if (!tooClose)
                        {
                            CarveChamberWithConnections(cell.x, cell.y, 3);
                            _chamberCenters.Add(cell);
                            chambersCreated++;
                        }
                    }
                }
            ...
            // Step 5: Ensure exit chamber exists
            ...
            Debug.Log($"[GridMazeGenerator] FINAL - Walls: {wallCount} ({wallCount * 100 / total}%), Corridors: {corridorCount} ({corridorCount * 100 / total}%), Rooms: {roomCount} ({roomCount * 100 / total}%)");
        }

        /// <summary>
        /// Carve a chamber with guaranteed corridor connections.
        /// Preserves corridor "stubs" that connect to this chamber.
        /// </summary>
        private void CarveChamberWithConnections(int centerX, int centerY, int size)
        {
            int halfSize = size / 2;

            // Step 1: Find which cardinal directions have corridors (these are connections)
            bool hasNorth = (centerY - halfSize - 1 >= 0 && _grid[centerX, centerY - halfSize - 1] == GridMazeCell.Corridor);
            bool hasEast  = (centerX + halfSize + 1 < _gridSize && _grid[centerX + halfSize + 1, centerY] == GridMazeCell.Corridor);
            bool hasSouth = (centerY + halfSize + 1 < _gridSize && _grid[centerX, centerY + halfSize + 1] == GridMazeCell.Corridor);
            bool hasWest  = (centerX - halfSize - 1 >= 0 && _grid[centerX - halfSize - 1, centerY] == GridMazeCell.Corridor);

            // Step 2: Carve the chamber (3x3 or 5x5 room)
            ...
            // Step 3: Preserve corridor stubs (entrance/exit points)
            // These are the cells where corridors connect to the chamber
            if (hasNorth) _grid[centerX, centerY - halfSize] = GridMazeCell.Room; // North entrance
            if (hasEast)  _grid[centerX + halfSize, centerY] = GridMazeCell.Room; // East entrance
            if (hasSouth) _grid[centerX, centerY + halfSize] = GridMazeCell.Room; // South entrance
            if (hasWest)  _grid[centerX - halfSize, centerY] = GridMazeCell.Room; // West entrance

            // Step 4: If no connections found, carve one eastward (fallback for isolated chambers)
            if (!hasNorth && !hasEast && !hasSouth && !hasWest)
            {
                if (centerX + halfSize + 1 < _gridSize && _grid[centerX + halfSize + 1, centerY] == GridMazeCell.Wall)
                {
                    _grid[centerX + halfSize + 1, centerY] = GridMazeCell.Corridor;
                }
            }
        }
        ...
        /// <summary>
        /// Check if cell is a corridor intersection (2+ ways for better chamber distribution).
        /// Uses 4 cardinal directions only for proper maze connectivity.
        /// </summary>
        /// <param name="x">X coordinate</param>
        /// <param name="y">Y coordinate</param>
        /// <param name="use8Way">Ignored - kept for API compatibility</param>
        private bool IsIntersection(int x, int y, bool use8Way = false)
        {
            ...
            return neighbors >= 2; // 2+ ways = intersection (was 3, too strict)
        }
