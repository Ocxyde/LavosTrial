...
// MAIN GAME ORCHESTRATOR - Handles maze generation, player, and game state
...
//
// GAME PROGRESSION:
// - Level 0: 12x12 maze (easy)
// - Level 1: 13x13 maze
// - Level N: (12+N)x(12+N) maze
...
namespace Code.Lavos.Core
{
    ...
    /// Handles maze generation, player spawning, and game state.
    ...
    public class CompleteMazeBuilder : MonoBehaviour
    {
        ...
        [Tooltip("Auto-finds PlayerController in scene")]
        [SerializeField] private PlayerController playerController;

        [Header("🎮 Game State")]
        [Tooltip("Current game level (affects maze size)")]
        [SerializeField] private int currentLevel = 0;
        
        [Tooltip("Current seed (longer = harder)")]
        ...
        private GridMazeGenerator gridMazeGenerator;  // Created programmatically (not a MonoBehaviour)
        private float cellSize;
        private float wallHeight;
        private uint computedSeed;
        private Vector3 spawnPosition;
        private Vector2Int spawnCell;
        private int currentMazeSize;  // Calculated from level
        ...
        private void Awake()
        {
            // Singleton pattern (scene-based, not DontDestroyOnLoad)
            if (_instance != null && _instance != this)
            {
                Debug.LogWarning("[CompleteMazeBuilder] ⚠️ Multiple instances detected - destroying duplicate");
                Destroy(gameObject);
                return;
            }
            ...
            // Calculate maze size from level
            CalculateMazeSize();
            ...
            computedSeed = ComputeSeed(currentSeed);
            Log($"[CompleteMazeBuilder] 🎲 Seed: {computedSeed} (Level {currentLevel}, Maze {currentMazeSize}x{currentMazeSize})", true);
        }
        ...
        /// <summary>
        /// Calculate maze size based on current level.
        /// Level 0 = 12x12, Level 1 = 13x13, etc.
        /// </summary>
        private void CalculateMazeSize()
        {
            // Base size is 12x12 at level 0
            // Increases by 1 for each level
            currentMazeSize = 12 + currentLevel;
            
            // Clamp to reasonable limits
            currentMazeSize = Mathf.Clamp(currentMazeSize, 12, 51);
            
            Log($"[CompleteMazeBuilder] 📏 Maze size calculated: {currentMazeSize}x{currentMazeSize} (Level {currentLevel})", true);
        }
        ...
        public void NextLevel()
        {
            currentLevel++;
            CalculateMazeSize();
            Log($"[CompleteMazeBuilder] 🎮 Level {currentLevel} - Maze {currentMazeSize}x{currentMazeSize}", true);
        }

        /// <summary>
        /// Set seed for next generation.
        /// Longer seed = harder maze.
        /// </summary>
        public void SetSeed(string seed)
        {
            currentSeed = seed;
            computedSeed = ComputeSeed(seed);
            Log($"[CompleteMazeBuilder] 🎲 Seed set: {seed} → {computedSeed}", true);
        }
        ...
        private void LoadConfig()
        {
            ...
            Log($"[CompleteMazeBuilder] 📖 Config loaded from JSON", true);
        }

        #endregion

        #region Public API - Main Game Methods

        /// <summary>
        /// Generate complete maze (call from editor menu or runtime).
        /// MAIN GAME ENTRY POINT - handles everything.
        /// </summary>
        ...
        public void GenerateMaze()
        {
            ...
            Log("[CompleteMazeBuilder] ════════════════════════════════════════");
            Log($"[CompleteMazeBuilder] ✅ Level {currentLevel} complete! Maze {currentMazeSize}x{currentMazeSize}", true);
            Log("[CompleteMazeBuilder] ════════════════════════════════════════");
        }

        /// <summary>
        /// Reload current scene (for testing/restart).
        /// </summary>
        public void ReloadScene()
        {
            Log("[CompleteMazeBuilder] 🔄 Reloading scene...");
        ...
        /// NOTE: GridMazeGenerator is created programmatically (not a MonoBehaviour)
        /// </summary>
        private void FindComponents()
        {
            Log("[CompleteMazeBuilder] 🔌 Finding components (plug-in-out)...", true);

            // GridMazeGenerator is created programmatically in CreateEntranceRoom()
            // It's a plain C# class, not a MonoBehaviour
        ...
        private void SpawnGround()
        {
            ...
            // Center under maze
            float mazeWidth = currentMazeSize * cellSize;
            float mazeHeight = currentMazeSize * cellSize;
        ...
        private void CreateEntranceRoom()
        {
            ...
            // Create grid generator programmatically
            gridMazeGenerator = new GridMazeGenerator();
            
            // Override grid size based on level
            gridMazeGenerator.gridSize = currentMazeSize;
            gridMazeGenerator.roomSize = GameConfig.Instance.defaultRoomSize;
            gridMazeGenerator.corridorWidth = GameConfig.Instance.defaultCorridorWidth;
            ...
            spawnPosition = new Vector3(
                ...
                GameConfig.Instance.defaultPlayerEyeHeight,  // FPS eye height
            ...
            Log($"[CompleteMazeBuilder] ✅ Grid maze generated ({currentMazeSize}x{currentMazeSize})", true);
        }
        ...
        private void SaveMaze()
        {
            ...
            // Save to database
            MazeSaveData.SaveGridMaze((int)computedSeed, gridMazeGenerator.SerializeToBytes(), spawnCell.x, spawnCell.y);
        ...
        private void SpawnPlayer()
        {
            ...
            playerController.transform.position += new Vector3(offsetX, 0f, offsetZ);

            // Ensure FPS camera is at eye level (middle eyes view) - loaded from JSON config
            var cameraTransform = playerController.GetComponentInChildren<Camera>()?.transform;
