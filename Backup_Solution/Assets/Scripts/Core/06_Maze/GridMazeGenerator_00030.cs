...
namespace Code.Lavos.Core
{
    ...
    public class GridMazeGenerator
    {
        ...
        private void CarveMazeWithDfs()
        {
            ...
            int stepSize = 2; // Move 2 cells at a time (leaves walls between)
            ...
            // Carve spawn chamber
            CarveChamber(startX, startY, 3);
            _spawnChamberCenter = new Vector2Int(startX, startY);

            int carvedCells = 0;
            int targetCells = (_gridSize * _gridSize) / 5; // Carve ~20% of grid
            ...
            while (stack.Count > 0 && carvedCells < targetCells)
            {
                ...
                for (int i = 0; i < numDirections; i++)
                {
                    ...
                    {
                        // Check if 2 cells away is also wall (not already carved)
                        int midX = current.x + dx[i];
                        int midY = current.y + dy[i];

                        if (_grid[midX, midY] == GridMazeCell.Wall)
                        {
                ...
                if (unvisitedDirections.Count > 0)
                {
                    ...
                    int midX = current.x + dx[dirIndex];
                    int midY = current.y + dy[dirIndex];

                    // Carve corridor to neighbor (including middle cell)
                    _grid[midX, midY] = GridMazeCell.Corridor;
        ...
        /// <summary>
        /// Carve a chamber (square area) at position.
        /// </summary>
        private void CarveChamber(int centerX, int centerY, int size)
        {
            int halfSize = size / 2;
