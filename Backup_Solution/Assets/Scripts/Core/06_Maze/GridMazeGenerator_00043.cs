...
namespace Code.Lavos.Core
{
    ...
    public sealed class GridMazeGenerator
    {
        ...
        //    Crossing a wall = +100 (heavy penalty, prefer open paths)
        // ─────────────────────────────────────────────────────────
                                        ...
                                        int sx, int sz, int ex, int ez)
        {
            ...
            while (open.Count > 0)
            {
                ...
                foreach (var dir in Direction8Helper.All)
                {
                    ...
                    // Wall crossing penalty
                    int wallPenalty = d.HasWall(current.X, current.Z, dir) ? 100 : 0;
                    int g = current.G + moveCost + wallPenalty;
