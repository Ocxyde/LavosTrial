...
namespace Code.Lavos.Core
{
    ...
    public class GridMazeGenerator
    {
        ...
        private void PlaceSpawnRoom()
        {
            ...
            Debug.Log($"[GridMazeGenerator]  Spawn point: ({spawnPointX}, {spawnPointY}) in room coordinates");
                ...
                for (int dy = 0; dy < spawnRoomSize; dy++)
                {
                    ...
                    if (gridX >= 0 && gridX < gridSize && gridY >= 0 && gridY < gridSize)
                    {
                        ...
                        if (dx == spawnPointX && dy == spawnPointY)
                        {
                            ...
                            Debug.Log($"[GridMazeGenerator]  SpawnPoint (CENTER) at grid ({gridX}, {gridY})");
                            cellsMarked++;
                        }
            ...
            Debug.Log($"[GridMazeGenerator]  ONE OPENING: {openWallName} side (for door)");
            Debug.Log($"[GridMazeGenerator]  Player protected: Walls on 3 sides (perimeter)");
        }
        ...
        private void PlaceOtherRooms()
        {
            ...
            var cfg = GameConfig.Instance;
            int baseRooms = UnityEngine.Random.Range(cfg.baseRoomMin, cfg.baseRoomMax);
            ...
            Debug.Log($"[GridMazeGenerator]  Grid size: {gridSize}, Room size: {roomSize}");
            ...
            for (int i = 0; i < additionalRooms; i++)
            {
                ...
                {
                    Debug.LogWarning($"[GridMazeGenerator]  Could not place room {i+1} (no valid position)");
                    roomsFailed++;
                }
            }

            Debug.Log($"[GridMazeGenerator]  Rooms placed: {roomsPlaced}, Failed: {roomsFailed}");
            Debug.Log($"[GridMazeGenerator]  Total room centers: {roomCenters.Count}");
            Debug.Log($"[GridMazeGenerator]  ==================================");
        }
        ...
        {
            int attempts = 100;
            int maxPos = gridSize - roomSize;

            if (maxPos <= 0)
            {
                Debug.LogError("[GridMazeGenerator] Room size too large for grid");
                return new Vector2Int(-1, -1);
            }

            for (int i = 0; i < attempts; i++)
            {
                int x = UnityEngine.Random.Range(1, maxPos);
                int y = UnityEngine.Random.Range(1, maxPos);

                // Skip spawn room area and its corridors
                if (IsInSpawnCorridorArea(x, y))
                {
                    Debug.Log($"[GridMazeGenerator]    Attempt {i+1}: ({x},{y}) - SKIPPED (in spawn corridor area)");
                    continue;
                }

                // Check if position overlaps existing rooms
                bool overlaps = false;
                for (int rx = x; rx < x + roomSize; rx++)
                {
                    for (int ry = y; ry < y + roomSize; ry++)
                    {
                        if (grid[rx, ry] == GridMazeCell.Room ||
                            grid[rx, ry] == GridMazeCell.SpawnPoint)
                        {
                            overlaps = true;
                            break;
                        }
                ...
                if (overlaps)
                {
                    Debug.Log($"[GridMazeGenerator]    Attempt {i+1}: ({x},{y}) - OVERLAPS existing room");
                }
                else
                {
                    Debug.Log($"[GridMazeGenerator]    Attempt {i+1}: ({x},{y}) - VALID!");
                    return new Vector2Int(x, y);
                }
            }

            Debug.LogWarning($"[GridMazeGenerator]  No valid position found after {attempts} attempts");
            return new Vector2Int(-1, -1);
        }

        /// <summary>
        /// Check if position is in spawn room corridor area.
        /// Only blocks a limited zone around the spawn room to avoid over-restricting placement.
        /// </summary>
        private bool IsInSpawnCorridorArea(int x, int y)
        {
            // Spawn room center
            int spawnX = spawnRoomCenter.x;
            int spawnY = spawnRoomCenter.y;

            // Corridor width (how wide is the corridor)
            int halfWidth = corridorWidth / 2;

            // Only block corridor area within a limited distance from spawn room
            // This prevents the corridor from blocking the entire grid width
            int corridorReach = Mathf.Max(roomSize * 2, 6);  // Block 2 room-lengths or minimum 6 cells

            // West corridor (limited reach from spawn room)
            if (x >= spawnX - corridorReach && x < spawnX && Mathf.Abs(y - spawnY) <= halfWidth)
            {
                return true;  // In west corridor zone
            }

            // East corridor (limited reach from spawn room)
            if (x > spawnX && x <= spawnX + corridorReach && Mathf.Abs(y - spawnY) <= halfWidth)
            {
                return true;  // In east corridor zone
            }

            return false;  // Not in spawn corridors
        }
