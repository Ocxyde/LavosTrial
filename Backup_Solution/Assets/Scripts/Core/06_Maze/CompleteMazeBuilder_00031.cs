...
namespace Code.Lavos.Core
{
    ...
    public class CompleteMazeBuilder : MonoBehaviour
    {
        ...
        public void GenerateMaze()
        {
            ...
            Log($"[CompleteMazeBuilder] 🎮 LEVEL {currentLevel} - {mazeSize}x{mazeSize}", true);
            Log($"[CompleteMazeBuilder] ════════════════════════════════════════");
            ...
            FindComponents();
            CleanupOldMaze();
            Log("[CompleteMazeBuilder] 🧹 STEP 1: Cleanup", true);

            SpawnGround();
            Log("[CompleteMazeBuilder] 🌍 STEP 2: Ground", true);

            GenerateGrid();
            Log("[CompleteMazeBuilder] 🏛️ STEP 3: Spawn Room", true);

            PlaceWalls();
            Log("[CompleteMazeBuilder] 🧱 STEP 4: Walls", true);

            PlaceDoors();
            Log("[CompleteMazeBuilder] 🚪 STEP 5: Doors", true);

            PlaceTorches();
            Log("[CompleteMazeBuilder] 🔥 STEP 6: Torches", true);

            SaveMaze();
            Log("[CompleteMazeBuilder] 💾 STEP 7: Saved", true);

            if (Application.isPlaying)
            {
                SpawnPlayer();
                Log($"[CompleteMazeBuilder] 👤 STEP 8: Player", true);
            }

            Log($"[CompleteMazeBuilder] ✅ Level {currentLevel} complete!", true);
        }
        ...
        private void SpawnGround()
        {
            ...
            Log($"[CompleteMazeBuilder] ✅ Ground {size}m x {size}m");
        }
        ...
        private void GenerateGrid()
        {
            ...
            Log($"[CompleteMazeBuilder] 🎯 SpawnPoint: {spawnCell}");
        }
        ...
        private void PlaceWalls()
        {
            ...
            Log($"[CompleteMazeBuilder] 🧱 {spawned} walls placed");
        }
        ...
        private void SpawnWall(Vector3 pos, Quaternion rot, string name, Transform parent)
        {
            if (wallPrefab == null)
            {
                LogError($"[CompleteMazeBuilder] ❌ Wall prefab missing! Run Tools → Quick Setup Prefabs");
                return;
            }
        ...
        private void PlaceDoors()
        {
            ...
            if (doorPos.x >= 0)
            {
                ...
                if (doorPrefab != null)
                {
                    ...
                    Log($"[CompleteMazeBuilder] 🚪 Door at ({doorPos.x}, {doorPos.y})");
                }
            }
        }
        ...
        private void PlaceTorches()
        {
            if (torchPrefab == null)
            {
                LogWarning("[CompleteMazeBuilder] ⚠️ Torch prefab missing");
                return;
            }
            ...
            Log($"[CompleteMazeBuilder] 🔥 {placed} torches mounted");
        }
        ...
        private void SpawnPlayer()
        {
            ...
            if (playerController == null)
            {
                LogWarning("[CompleteMazeBuilder] ⚠️ PlayerController missing - add to scene");
                return;
            }
            ...
            Log($"[CompleteMazeBuilder] 👤 Player at {playerController.transform.position}");
        }
