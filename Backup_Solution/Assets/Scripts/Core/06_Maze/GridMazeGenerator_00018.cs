...
namespace Code.Lavos.Core
{
    ...
    public class GridMazeGenerator
    {
        ...
        /// <summary>
        /// Step 4: Carve a corridor from spawn room to maze edge (player exit).
        /// The DFS already created the maze passages, we just need to ensure
        /// the spawn room connects to the main maze area.
        /// </summary>
        private void CarveExitCorridor()
        {
            Debug.Log($"[GridMazeGenerator] STEP 4: Carving exit corridor from spawn room...");

            // Carve corridor from spawn room open wall toward the maze
            int halfWidth = corridorWidth / 2;
            Vector2Int dir = spawnEntranceDirection;

            // Start from spawn center, carve toward maze edge
            int startX = spawnRoomCenter.x;
            int startY = spawnRoomCenter.y;

            // Carve in the entrance direction until we hit a Floor cell (DFS passage)
            ...
            while (steps < maxSteps)
            {
                ...
                {
                    int cx = dir.x == 0 ? startX + w : startX;
                    int cy = dir.y == 0 ? startY + w : startY;
