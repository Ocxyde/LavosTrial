...
namespace Code.Lavos.Core
{
    ...
    public class CompleteMazeBuilder : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // Generate random seed from system tick count (truly random each load)
            seed = (uint)System.Environment.TickCount ^ (uint)System.Guid.NewGuid().GetHashCode();
            seed = (uint)(seed & 0x7FFFFFFF); // Ensure positive int range
            ...
            Log($"[CompleteMazeBuilder] Level {currentLevel} - Maze {mazeSize}x{mazeSize} - Seed: {seed}");
        }
