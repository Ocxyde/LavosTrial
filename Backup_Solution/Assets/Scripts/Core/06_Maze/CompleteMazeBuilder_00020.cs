...
namespace Code.Lavos.Core
{
    ...
    public class CompleteMazeBuilder : MonoBehaviour
    {
        ...
        /// 4. ROOMS - Mark room cells in grid (3x3 or 5x5 clear areas)
        /// 5. CORRIDORS - Mark corridor cells in grid (2-cell wide paths)
        /// 6. OUTER WALLS - Mark perimeter cells as walls
        /// 7. READ GRID - Spawn walls ONLY where grid says walls
        /// 8. DOORS - Place doors in marked door positions
        /// 9. OBJECTS - Place torches, chests, enemies, items
        /// 10. SAVE - Save grid to database (byte-by-byte)
        /// 11. PLAYER - Spawn in marked spawn cell
        /// </summary>
        public void GenerateMazeGeometryOnly()
        {
            ...
            Debug.Log($"[CompleteMazeBuilder] 🔲 Step 3: Empty grid created ({configCache.mazeWidth}x{configCache.mazeHeight})");

            // STEP 4: ROOMS - Mark room cells in grid (3x3 or 5x5 clear areas)
            // Already done in CreateVirtualGridAndPlaceRooms()
            Debug.Log($"[CompleteMazeBuilder] 🏛️ Step 4: Rooms marked in grid at {entranceRoomCell}");

            // STEP 5: CORRIDORS - Mark corridor cells in grid (2-cell wide paths)
            GenerateCorridors();
            Debug.Log("[CompleteMazeBuilder] 🔨 Step 5: Corridors marked in grid");

            // STEP 6: OUTER WALLS - Mark perimeter cells as walls
            // Done in SpawnWallsFromGrid()
            Debug.Log("[CompleteMazeBuilder] 🧱 Step 6: Outer walls marked");

            // STEP 7: READ GRID - Spawn walls ONLY where grid says walls
            SpawnWallsFromGrid();
            Debug.Log("[CompleteMazeBuilder] 🧱 Step 7: Walls spawned from grid (byte-by-byte)");

            // STEP 8: DOORS - Place doors in marked door positions
            SpawnDoors();
            Debug.Log("[CompleteMazeBuilder] 🚪 Step 8: Doors placed");

            // STEP 9: OBJECTS - Place torches, chests, enemies, items
            PlaceObjects();
            Debug.Log("[CompleteMazeBuilder] 🎒 Step 9: Objects placed (torches, chests, enemies, items)");

            // STEP 10: SAVE - Save grid to database (byte-by-byte binary)
            SaveGridToDatabase();
            Debug.Log("[CompleteMazeBuilder] 💾 Step 10: Grid saved (byte-by-byte)");

            // [OPTIONAL] STEP 11: CEILING - Spawn ceiling (disabled for testing)
            // SpawnCeiling();
        ...
        private void CreateVirtualGridAndPlaceRooms()
        {
            ...
            gridMazeGenerator = new GridMazeGenerator();
            gridMazeGenerator.gridSize = configCache.mazeWidth;  // Use cached values
            gridMazeGenerator.roomSize = 5;  // 5x5 rooms (spacious)
            gridMazeGenerator.corridorWidth = 2;  // 2 cells wide

            Debug.Log($"[CompleteMazeBuilder] 🔲 Grid created: {configCache.mazeWidth}x{configCache.mazeHeight} (all cells = Floor initially)");
