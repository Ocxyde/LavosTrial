...
namespace Code.Lavos.Core
{
    ...
    public class CompleteMazeBuilder : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // Generate random seed from system entropy (truly random each session)
            // This seed is NOT saved - each play session gets a unique maze
            uint rawSeed = (uint)System.Environment.TickCount ^ (uint)System.Guid.NewGuid().GetHashCode();

            // Hash seed for better distribution (SHA256 -> first 4 bytes)
            ...
            Log("[CompleteMazeBuilder] Seed is procedural - new maze each session!");
        }
