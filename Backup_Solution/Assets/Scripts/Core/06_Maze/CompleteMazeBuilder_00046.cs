...
namespace Code.Lavos.Core
{
    ...
    public class CompleteMazeBuilder : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // Higher seed value = larger maze, more rooms, more complexity
            float seedFactor = seed / (float)int.MaxValue; // 0.0 to 1.0
            int bonusSize = Mathf.FloorToInt(seedFactor * 10); // 0-10 extra size
            int bonusRooms = Mathf.FloorToInt(seedFactor * 5); // 0-5 extra rooms per level

            mazeSize = 12 + currentLevel + bonusSize;
            mazeSize = Mathf.Clamp(mazeSize, 12, 51);

            // Pass seed factor to grid generator for room/complexity scaling
