...
namespace Code.Lavos.Core
{
    ...
    public class MazeTorchTest : MonoBehaviour
    {
        ...
        private void SpawnTestPlayer()
        {
            ...
            // Spawn player INSIDE the maze (in a corridor, not in walls)
            // Maze cells are 6m size, walls at odd indices, corridors at even indices
            // Start at cell (1,1) which is a guaranteed corridor
            float cellSize = 6f;
            float spawnX = 1f * cellSize; // Cell 1 in X
            float spawnZ = 1f * cellSize; // Cell 1 in Z
            ...
            Log($"[MazeTorchTest] Player spawned at {_testPlayer.transform.position} (INSIDE maze corridor)");
