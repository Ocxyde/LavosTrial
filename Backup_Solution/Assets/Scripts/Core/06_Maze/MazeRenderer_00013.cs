...
namespace Code.Lavos.Core
{
    ...
    public class MazeRenderer : MonoBehaviour
    {
        ...
        /// Supports 8-way walls (N, NE, E, SE, S, SW, W, NW).
                ...
                for (int y = 0; y < mazeSize; y++)
                {
                    ...
                    if (y > 0)
                    {
                        ...
                        if (NeedsWallBetween(current, north))
                        {
                            ...
                            SpawnWall(pos, WallDirection.North, $"Wall_N_{x}_{y}", parent);
                            spawned++;
                            wallPlacedCount++;
                        }
                    }

                    // Check SOUTH boundary (180°)
                    if (y + 1 < mazeSize)
                    {
                        ...
                        if (NeedsWallBetween(current, south))
                        {
                            ...
                            SpawnWall(pos, WallDirection.South, $"Wall_S_{x}_{y}", parent);
                            spawned++;
                            wallPlacedCount++;
                        }
                    }

                    // Check WEST boundary (270°)
                    if (x > 0)
                    {
                        ...
                        if (NeedsWallBetween(current, west))
                        {
                            ...
                            SpawnWall(pos, WallDirection.West, $"Wall_W_{x}_{y}", parent);
                            spawned++;
                            wallPlacedCount++;
                        }
                    }

                    // Check EAST boundary (90°)
                    if (x + 1 < mazeSize)
                    {
                        ...
                        if (NeedsWallBetween(current, east))
                        {
                            ...
                            SpawnWall(pos, WallDirection.East, $"Wall_E_{x}_{y}", parent);
                            spawned++;
                            wallPlacedCount++;
                        }
                    }

                    // Check DIAGONAL boundaries (45°, 135°, 225°, 315°)
                    CheckAndSpawnDiagonalWall(x, y, 1, 1, WallDirection.NorthEast, ref spawned, ref wallPlacedCount, ref skippedCount, parent);
                    CheckAndSpawnDiagonalWall(x, y, 1, -1, WallDirection.SouthEast, ref spawned, ref wallPlacedCount, ref skippedCount, parent);
                    CheckAndSpawnDiagonalWall(x, y, -1, 1, WallDirection.NorthWest, ref spawned, ref wallPlacedCount, ref skippedCount, parent);
                    CheckAndSpawnDiagonalWall(x, y, -1, -1, WallDirection.SouthWest, ref spawned, ref wallPlacedCount, ref skippedCount, parent);
                }
            }
        ...
        /// <summary>
        /// Check and spawn diagonal wall if needed.
        /// </summary>
        private void CheckAndSpawnDiagonalWall(int x, int y, int dx, int dy, WallDirection direction, 
            ref int spawned, ref int wallPlacedCount, ref int skippedCount, Transform parent)
        {
            int nx = x + dx;
            int ny = y + dy;
            ...
            {
                GridMazeCell current = grid.GetCell(x, y);
                GridMazeCell neighbor = grid.GetCell(nx, ny);

                // Spawn diagonal wall if one is walkable and other is not
                bool currentWalkable = (current == GridMazeCell.Room || current == GridMazeCell.Corridor);
                bool neighborWalkable = (neighbor == GridMazeCell.Room || neighbor == GridMazeCell.Corridor);

                if (currentWalkable != neighborWalkable)
                {
                    // Position at diagonal boundary (corner of cell)
                    Vector3 pos = new Vector3(
                        (x + (dx > 0 ? 1 : 0)) * cellSize,
                        wallHeight / 2f,
                        (y + (dy > 0 ? 1 : 0)) * cellSize
                    ...
                    SpawnWall(pos, direction, $"Wall_{direction}_{x}_{y}", parent);
                    spawned++;
                    wallPlacedCount++;
                }
            }
