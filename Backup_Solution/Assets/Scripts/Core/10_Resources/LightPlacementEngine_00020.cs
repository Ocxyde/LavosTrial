...
namespace Code.Lavos.Core
{
    ...
    public class LightPlacementEngine : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // If still null, try to load from GameConfig8 (8-axis maze)
            if (torchPrefab == null)
            {
                var cfg8 = FindFirstObjectByType<GameConfig8>();
                if (cfg8 != null && !string.IsNullOrEmpty(cfg8.torchPrefab))
                {
                    Debug.Log($"[LightPlacementEngine]  Loading torch prefab from GameConfig8: {cfg8.torchPrefab}");
                    torchPrefab = Resources.Load<GameObject>(cfg8.torchPrefab.Replace("Assets/Resources/", "").Replace(".prefab", ""));

                    if (torchPrefab != null)
                    {
                        Debug.Log($"[LightPlacementEngine]  TorchPrefab loaded: {torchPrefab.name}");
                        return; // Found in GameConfig8
                    }
                    else
                    {
                        Debug.LogWarning($"[LightPlacementEngine]  TorchPrefab not found at: {cfg8.torchPrefab}");
                    }
                }
            }

            // Fallback to GameConfig (standard maze)
            if (torchPrefab == null)
            {
