...
namespace Code.Lavos.Core
{
    ...
    public class TorchPool : MonoBehaviour
    {
        // ─── Single Torch Prefab ───────────────────────────────────────────────
        [Header("Torch Prefab")]
        [Tooltip("Torch prefab with LightSocket + EffectSocket")]
        [SerializeField] private GameObject torchEmissivePrefab;
                                   ...
                                   Material flameMat, Material handleMat)
        {
            ...
            if (torchEmissivePrefab != null)
            {
                // Instance from prefab
                go = Instantiate(torchEmissivePrefab, parent);
            }
            else
            {
                // Create new torch
