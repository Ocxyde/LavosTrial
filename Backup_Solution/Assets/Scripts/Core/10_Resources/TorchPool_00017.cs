...
namespace Code.Lavos.Core
{
    ...
    public class TorchPool : MonoBehaviour
    {
                                   ...
                                   Material flameMat, Material handleMat)
        {
            ...
            if (useBraseroFlame)
            {
                // Generate unique GUID for this torch instance
                string torchGUID = System.Guid.NewGuid().ToString();
                go.name = $"Torch_{torchGUID}";
                
                Debug.Log($"[TorchPool] Creating torch {torchGUID} at {position}");
                ...
                Debug.Log($"[TorchPool] Torch {torchGUID} - Particles: {effectSocket != null}, Light: {light != null}");
                ...
                if (light != null)
                {
                    ...
                    {
                        effectSocket.Play();
                        Debug.Log($"[TorchPool] ✅ Torch {torchGUID} ON + Particles playing");
                    }
                    else
                    {
                        Debug.Log($"[TorchPool] ✅ Torch {torchGUID} ON (no particles)");
                    }
                    
                    // Record in Architect RAM with GUID
                    WallPositionArchitect.RecordTorchWithGUID(position, rotation, torchY, torchInset, torchGUID);
                }
                else
                {
                    Debug.LogWarning($"[TorchPool] ❌ Torch {torchGUID} Light missing!");
