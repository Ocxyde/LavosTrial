    ...
    public class TorchPool : MonoBehaviour
    {
                                   ...
                                   Material flameMat, Material handleMat)
        {
            ...
            if (useBraseroFlame)
            {
                ...
                // EffectSocket has ParticleSystem for flame effect
                var effectSocket = go.transform.Find("EffectSocket");
                var particleSystem = effectSocket?.GetComponent<ParticleSystem>();
                
                // LightSocket is child of EffectSocket - use recursive search
                Transform lightSocketTransform = null;
                if (effectSocket != null)
                {
                    // Search in EffectSocket's children
                    lightSocketTransform = effectSocket.Find("LightSocket");
                    
                    // If not found, search recursively in all children
                    if (lightSocketTransform == null)
                    {
                        foreach (Transform child in effectSocket)
                        {
                            if (child.name == "LightSocket")
                            {
                                lightSocketTransform = child;
                                break;
                            }
                        }
                    }
                ...
                var light = lightSocketTransform?.GetComponent<Light>();

                Debug.Log($"[TorchPool] Torch components - EffectSocket: {effectSocket != null}, LightSocket: {lightSocketTransform != null}, Light: {light != null}");
