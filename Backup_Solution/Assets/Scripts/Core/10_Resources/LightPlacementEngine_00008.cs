...
namespace Code.Lavos.Core
{
    ...
    public class LightPlacementEngine : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // Auto-load torch prefab from JSON config if not assigned
            if (torchPrefab == null)
            {
                Debug.Log("[LightPlacementEngine] 🔧 TorchPrefab not assigned - loading from GameConfig-default.json...");
                
                var config = GameConfig.Instance;
                if (!string.IsNullOrEmpty(config.torchPrefab))
                {
                    torchPrefab = Resources.Load<GameObject>(config.torchPrefab.Replace("Prefabs/", "").Replace(".prefab", ""));
                    
                    // If not in Resources, try direct load
                    if (torchPrefab == null)
                    {
                        torchPrefab = AssetDatabase.LoadAssetAtPath<GameObject>("Assets/" + config.torchPrefab);
                    }
                    
                    if (torchPrefab != null)
                    {
                        Debug.Log($"[LightPlacementEngine] ✅ TorchPrefab loaded from config: {config.torchPrefab}");
                    }
                    else
                    {
                        Debug.LogWarning($"[LightPlacementEngine] ⚠️ Could not load torch prefab from config path: {config.torchPrefab}");
                    }
                }
            }

            // Auto-find torch prefab from TorchPool if still not assigned
            if (torchPrefab == null)
            {
            ...
            if (torchPrefab == null)
            {
                ...
                Debug.LogError("[LightPlacementEngine] ❌ Please assign in Inspector OR add to GameConfig-default.json OR add TorchPool component");
            }
