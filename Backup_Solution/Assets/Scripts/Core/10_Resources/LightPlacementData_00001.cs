...
namespace Code.Lavos.Core
{
    ...
    public static class LightPlacementData
    {
        ...
        public static byte[] SaveTorches(List<WallPositionArchitect.TorchRecord> torches, int seed)
        {
            ...
            // Encrypt entire buffer (except header magic for validation)
            EncryptBuffer(buffer, HEADER_SIZE, seed);
        ...
        public static List<WallPositionArchitect.TorchRecord> LoadTorches(byte[] data, int seed)
        {
            ...
            // Decrypt (skip magic number for validation)
            DecryptBuffer(buffer, HEADER_SIZE, seed);
        ...
        /// <summary>
        /// Generate encryption key from seed using simple hash.
        /// </summary>
        private static byte[] GenerateKey(int seed)
        {
            ...
            for (int i = 0; i < 256; i++)
            {
                ...
                state = checked(state * 1103515245 + 12345);
        ...
        /// <summary>
        /// Encrypt buffer using XOR cipher (skip first skipBytes for validation).
        /// </summary>
        private static void EncryptBuffer(byte[] buffer, int skipBytes, int seed)
        {
            byte[] key = GenerateKey(seed);
            int keyIndex = 0;
            
            for (int i = skipBytes; i < buffer.Length; i++)
            {
        ...
        /// <summary>
        /// Decrypt buffer using XOR cipher (skip first skipBytes for validation).
        /// </summary>
        private static void DecryptBuffer(byte[] buffer, int skipBytes, int seed)
        {
            // XOR is symmetric - same operation for encrypt/decrypt
            EncryptBuffer(buffer, skipBytes, seed);
        }
