...
namespace Code.Lavos.Core
{
    ...
    public class LightPlacementEngine : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (torchPrefab == null)
            {
                // Try to find TorchPool component on same GameObject
            ...
            if (torchPrefab == null)
            {
                ...
                var torchInScene = FindFirstObjectByType<GameObject>();
                if (torchInScene != null && torchInScene.name.Contains("Torch"))
                {
                    torchPrefab = torchInScene;
                    Debug.Log("[LightPlacementEngine] ✅ TorchPrefab found in scene");
                }
            }
            ...
            if (torchPrefab == null)
            {
                ...
                Debug.LogError("[LightPlacementEngine]    1. TorchPool has torchHandlePrefab assigned, OR");
                Debug.LogError("[LightPlacementEngine]    2. TorchHandlePrefab exists in Resources folder, OR");
                Debug.LogError("[LightPlacementEngine]    3. A torch exists in the scene");
