...
namespace Code.Lavos.Core
{
    ...
    public class TorchController : MonoBehaviour
    {
        ...
        /// <summary>
        /// Turn torch ON - Activate flame and light emission.
        /// </summary>
        public void TurnOn()
        {
            if (_isOn) return;
            ...
            Debug.Log($"[TorchController] LightEngine.Instance = {(lightEngine != null ? "FOUND" : "NULL")}");
            ...
            {
                Debug.LogWarning($"[TorchController] LightEngine not found! Light will not emit.");
            }

            // Enable local light as fallback
            if (_light != null)
            {
                _light.enabled = true;
                _light.intensity = lightBaseIntensity;
                Debug.Log($"[TorchController] Local light enabled with intensity {lightBaseIntensity}");
            }

            Debug.Log($"[TorchController] {_lightId} turned ON");
        }
