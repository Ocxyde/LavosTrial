...
namespace Code.Lavos.Core
{
    ...
    public class TorchPool : MonoBehaviour
    {
                                   ...
                                   Material flameMat, Material handleMat)
        {
            ...
            if (useBraseroFlame)
            {
                // Find components in prefab (EffectSocket contains BraseroFlame + ParticleSystem)
                var effectSocket = go.transform.Find("EffectSocket");
                var braseroFlame = effectSocket?.GetComponent<BraseroFlame>();
                var particleSystem = effectSocket?.GetComponent<ParticleSystem>();
                
                // LightSocket is child of EffectSocket
                var lightSocket = effectSocket?.Find("LightSocket");
                var light = lightSocket?.GetComponent<Light>();

                Debug.Log($"[TorchPool] Torch components - EffectSocket: {effectSocket != null}, BraseroFlame: {braseroFlame != null}, Light: {light != null}");

                if (braseroFlame != null && light != null)
                {
                    ctrl.InitializeBrasero(light, braseroFlame);
                    ctrl.TurnOn();
                    Debug.Log($"[TorchPool] ✅ Torch initialized and turned ON");
                }
                else
                {
                    Debug.LogWarning($"[TorchPool] ❌ Torch components missing!");
