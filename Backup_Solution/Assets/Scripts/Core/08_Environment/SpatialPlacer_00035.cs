...
namespace Code.Lavos.Core
{
    ...
    public class SpatialPlacer : MonoBehaviour
    {
        ...
        public void PlaceTorches()
        {
            ...
            // Use binary storage system (no longer needs MazeRenderer)
            PlaceTorchesWithBinaryStorage();
        }
        ...
        /// 
        /// UPDATED: No longer requires MazeRenderer - uses GridMazeGenerator directly.
        /// </summary>
        private void PlaceTorchesWithBinaryStorage()
        {
            ...
            // Get wall faces from grid (no longer needs MazeRenderer)
        ...
        /// </summary>
        [System.Obsolete("Use PlaceTorchesWithBinaryStorage() instead - legacy causes teleportation")]
        private void PlaceTorchesLegacy()
        {
        ...
        /// <summary>
        /// Get wall faces from GridMazeGenerator.
        /// UPDATED: No longer uses MazeRenderer (deprecated).
        /// </summary>
        private List<(Vector3, Quaternion)> GetWallFacesFromGrid()
        {
            ...
            {
                Debug.LogError("[SpatialPlacer] GridMazeGenerator not found!");
                return wallFaces;
            }

            int size = gridMazeGenerator.GridSize;
            float cellSize = GameConfig.Instance.defaultCellSize;
            float wallHeight = GameConfig.Instance.defaultWallHeight;

            // Iterate through grid and find wall faces
            for (int x = 0; x < size; x++)
            {
                for (int y = 0; y < size; y++)
                {
                    ...
                    // Check adjacent cells for open faces (where torches can be placed)
                    // North face
            ...
            Debug.Log($"[SpatialPlacer] Found {wallFaces.Count} wall faces from grid");
            return wallFaces;
        }

        [System.Obsolete("Use GetWallFacesFromGrid() instead - MazeRenderer is deprecated")]
        ...
        Vector3 CellToWorld(Vector2Int cell)
        {
            ...
            float cellSize = GameConfig.Instance.defaultCellSize;  // From JSON config (NO HARDCODING!)
