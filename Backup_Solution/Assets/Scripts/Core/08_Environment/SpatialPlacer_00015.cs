    ...
    public class SpatialPlacer : MonoBehaviour
    {
        ...
        public void PlaceTorches()
        {
            ...
            foreach (var (wallPos, wallRot) in wallFaces)
            {
                ...
                // We need: torch positioned OUT from wall, facing OUTWARD
                ...
                // Step 5: Rotate torch to face OUTWARD (180° around Y from wall rotation)
                Quaternion torchRot = Quaternion.Euler(0f, 180f, 0f) * wallRot;

                // Debug: Log first torch
                if (torchesPlaced == 0)
                {
                    Debug.Log($"[SpatialPlacer] Wall normal: {wallNormal}, Inset: {torchInset}, Torch pos: {torchPos}, Torch rot: {torchRot.eulerAngles}");
                }
                // ═══════════════════════════════════════════════════════════
