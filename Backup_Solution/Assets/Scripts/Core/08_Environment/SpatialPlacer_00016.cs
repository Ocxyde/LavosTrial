    ...
    public class SpatialPlacer : MonoBehaviour
    {
        ...
        public void PlaceTorches()
        {
            ...
            foreach (var (wallPos, wallRot) in wallFaces)
            {
                ...
                // Step 5: Use wall rotation directly (no Y rotation)
                Quaternion torchRot = wallRot;

                // Debug: Log first torch
                if (torchesPlaced == 0)
                {
                    Debug.Log($"[SpatialPlacer] Torch pos: {torchPos}, Torch rot: {torchRot.eulerAngles}");
                }
                // ═══════════════════════════════════════════════════════════
                ...
                // Record torch position in architect's blueprint
                WallPositionArchitect.RecordTorch(torchPos, torchRot, torchY, torchInset);
            ...
            // Save architect's blueprint
            WallPositionArchitect.SaveBlueprint($"MazeBlueprint_{System.DateTime.Now:yyyyMMdd_HHmmss}.txt");
        }
