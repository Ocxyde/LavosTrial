    ...
    public class SpatialPlacer : MonoBehaviour
    {
        ...
        public void PlaceTorches()
        {
            ...
            foreach (var (wallPos, wallRot) in wallFaces)
            {
                ...
                // Calculate torch position and rotation
                // Wall rotation faces INTO the wall, we need torch to face OUTWARD
                float torchY = torchHeightRatio * GetWallHeight();
                
                // Position: at wall surface, at correct height
                Vector3 torchPos = new Vector3(wallPos.x, torchY, wallPos.z);
                
                // Rotate torch 180° to face outward from wall
                // wallRot faces INTO wall, so we rotate 180° around Y to face OUT
                Quaternion torchRot = wallRot * Quaternion.Euler(0f, 180f, 0f);
                
                // Move torch OUT from wall surface by inset distance
                // After 180° rotation, forward now points OUTWARD
                torchPos += torchRot * Vector3.forward * torchInset;

                // Debug: Log first torch position
                if (torchesPlaced == 0)
                {
                    Debug.Log($"[SpatialPlacer] Torch position: {torchPos}, rotation: {torchRot.eulerAngles}, wallRot: {wallRot.eulerAngles}");
                }
