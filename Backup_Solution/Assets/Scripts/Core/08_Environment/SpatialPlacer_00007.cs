...
namespace Code.Lavos.Core
{
    ...
    public class SpatialPlacer : MonoBehaviour
    {
        ...
        public void PlaceTorches()
        {
            ...
            int torchesPlaced = 0;
            int torchesAttempted = 0;
            int torchesSkippedDistance = 0;
            int torchesSkippedOther = 0;

            Debug.Log($"[SpatialPlacer] Starting torch placement - Target: {torchCount}, Wall faces: {wallFaces.Count}");
            ...
            foreach (var (wallPos, wallRot) in wallFaces)
            {
                ...
                if (Random.value > GetTorchProbability()) 
                {
                    torchesSkippedOther++;
                    continue;
                }
                ...
                // Get torch from pool
                var torchCtrl = torchPool.Get(torchPos, wallRot, torchRoot, flameFrames, flameMat, handleMat);
                placedTorches.Add(wallPos);
                torchesPlaced++;

                // Log EVERY torch spawn
                Debug.Log($"[SpatialPlacer] ✅ Torch #{torchesPlaced} at {torchPos:F2} (wall: {wallPos:F2})");

                if (torchesPlaced >= torchCount) 
                {
                    Debug.Log($"[SpatialPlacer] Reached target torch count: {torchCount}");
                    break;
                }
            }

            Debug.Log($"[SpatialPlacer] ═══ TORCH PLACEMENT SUMMARY ═══");
            Debug.Log($"[SpatialPlacer] Target: {torchCount} | Placed: {torchesPlaced}");
            Debug.Log($"[SpatialPlacer] Wall faces checked: {torchesAttempted}");
            Debug.Log($"[SpatialPlacer] Skipped (probability): {torchesSkippedOther}");
            Debug.Log($"[SpatialPlacer] Skipped (too close): {torchesSkippedDistance}");
