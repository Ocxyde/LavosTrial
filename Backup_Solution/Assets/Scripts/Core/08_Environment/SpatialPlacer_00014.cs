    ...
    public class SpatialPlacer : MonoBehaviour
    {
        ...
        public void PlaceTorches()
        {
            ...
            foreach (var (wallPos, wallRot) in wallFaces)
            {
                ...
                // ═══════════════════════════════════════════════════════════
                // TORCH PLACEMENT MATH - OUTWARD FROM WALL
                // ═══════════════════════════════════════════════════════════
                // wallPos = center of wall face
                // wallRot = rotation that faces INTO the wall
                // We need: torch facing OUTWARD, positioned OUT from wall surface
                
                // Step 1: Calculate torch height on wall
                float torchY = torchHeightRatio * GetWallHeight();
                
                // Step 2: Start at wall face center, at torch height
                Vector3 torchPos = new Vector3(wallPos.x, torchY, wallPos.z);
                
                // Step 3: wallRot.forward points INTO wall, negate to point OUT
                Vector3 wallNormal = -(wallRot * Vector3.forward);
                
                // Step 4: Move torch OUT from wall along normal by inset distance
                torchPos += wallNormal * torchInset;
                
                // Step 5: Rotate torch 180° so it faces OUTWARD (same direction as normal)
                Quaternion torchRot = Quaternion.LookRotation(wallNormal, Vector3.up);
                
                // Debug: Log first torch
                if (torchesPlaced == 0)
                {
                    Debug.Log($"[SpatialPlacer] Wall normal: {wallNormal}, Torch pos: {torchPos}, Torch rot: {torchRot.eulerAngles}");
                }
                // ═══════════════════════════════════════════════════════════
