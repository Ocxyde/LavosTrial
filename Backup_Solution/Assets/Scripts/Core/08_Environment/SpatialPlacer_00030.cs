...
namespace Code.Lavos.Core
{
    ...
    public class SpatialPlacer : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            // Update lighting based on maze level
            if (useDynamicLighting && lightingConfig != null)
            {
        ...
        /// <summary>
        /// Update lighting settings based on maze difficulty level.
        /// </summary>
        ...
        public void UpdateLightingForLevel(int mazeLevel)
        {
            ...
            cachedTorchCount = lightingConfig.GetTorchCountForLevel(mazeLevel);
            cachedLightRange = lightingConfig.GetLightRangeForLevel(mazeLevel);
            cachedLightIntensity = lightingConfig.GetLightIntensityForLevel(mazeLevel);
            
            Debug.Log($"[SpatialPlacer] 🕯️ Maze Level {mazeLevel}: {cachedTorchCount} torches, {cachedLightRange:F1}m range, {cachedLightIntensity:F2} intensity");
            Debug.Log($"[SpatialPlacer] 🕯️ Lighting Ratio: {lightingConfig.CalculateLightingRatio(mazeLevel):P0}");
        }
        ...
        private void PlaceTorchesWithBinaryStorage(MazeRenderer mazeRenderer)
        {
            ...
            // ALWAYS save to binary first (even if file exists, overwrite with latest)
            lightPlacementEngine.SaveTorches(mazeId, currentSeed, torchRecords);

            // Then load and instantiate from binary (no teleportation!)
            bool loaded = lightPlacementEngine.LoadAndInstantiateTorches(mazeId, currentSeed);

            if (!loaded)
            {
                // This shouldn't happen since we just saved, but fallback just in case
                Debug.LogError("[SpatialPlacer] Failed to load torches after saving! Falling back to batch instantiate");
                lightPlacementEngine.InstantiateTorchesBatch(torchRecords);
            }
        ...
        private void PlaceTorchesLegacy(MazeRenderer mazeRenderer)
        {
            ...
            var drawingPool = DrawingPool.Instance;
            if (drawingPool == null)
            {
                Debug.LogError("[SpatialPlacer] DrawingPool.Instance not found!");
                return;
            }

            var flameFrames = drawingPool.GetFlameTextures();
            if (flameFrames == null || flameFrames.Length == 0)
            {
                Debug.LogWarning("[SpatialPlacer] No flame textures available!");
                return;
            }

            // Get torch root transform (via reflection)
            var torchRoot = GetTorchRootFromTorchPool();
            ...
            foreach (var (wallPos, wallRot) in wallFaces)
            {
                ...
                // Spacing check
                bool tooClose = false;
                foreach (var p in placedTorches)
                {
                ...
                // Step 5: FLIP torch - flame faces OUTWARD from wall
                // Add 180° to Y so flame faces away from wall
                // Handle will be against the wall
                // Tilt 35° on X for better visibility
                Quaternion torchRot = Quaternion.Euler(35f, wallRot.eulerAngles.y + 180f, 0f);
                ...
                if (torchesPlaced >= torchCount) break;
            }
        ...
        {
            var type = GetComponent<MazeRenderer>().GetType();
            var field = type.GetField("wallHeight",
                System.Reflection.BindingFlags.NonPublic |
                System.Reflection.BindingFlags.Instance);

            return field != null ? (float)field.GetValue(GetComponent<MazeRenderer>()) : 3.5f;
        }
