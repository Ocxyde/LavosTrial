    ...
    public class SpatialPlacer : MonoBehaviour
    {
        ...
        public void PlaceTorches()
        {
            ...
            foreach (var (wallPos, wallRot) in wallFaces)
            {
                ...
                // Wall rotation faces INTO the wall, so we negate forward to go OUTWARD
                Vector3 outward = -(wallRot * Vector3.forward);
                float torchY = torchHeightRatio * GetWallHeight();
                // Position torch: at wall height, sticking OUT from wall surface
                Vector3 torchPos = new Vector3(wallPos.x, torchY, wallPos.z) + outward * torchInset;

                // Rotate torch to face OUTWARD from wall (180° from wall normal)
