    ...
    public class SpatialPlacer : MonoBehaviour
    {
        ...
        public void PlaceTorches()
        {
            ...
            foreach (var (wallPos, wallRot) in wallFaces)
            {
                ...
                if (randomValue > GetTorchProbability()) 
                {
                ...
                // Step 3: Get wall normal (pointing OUT from wall)
                ...
                // Debug: Log torch positions
                if (torchesPlaced < 5) // Log first 5 torches
                {
                    Debug.Log($"[SpatialPlacer] Torch #{torchesPlaced + 1} | Pos: {torchPos} | Rot: {torchRot.eulerAngles} | WallNormal: {wallNormal} | Inset: {torchInset}");
                }
                // ═══════════════════════════════════════════════════════════
            ...
            Debug.Log($"[SpatialPlacer] Loop finished - checked {wallFaces.Count} walls, placed {torchesPlaced} torches");
            ...
            {
                // Always log completion count
                Debug.Log($"[SpatialPlacer] ✅ Placement complete: {torchesPlaced}/{torchCount} torches placed");
            }
        }
        ...
        {
            Debug.Log($"[SpatialPlacer] RemoveTorches called - torchPool={torchPool != null}");
