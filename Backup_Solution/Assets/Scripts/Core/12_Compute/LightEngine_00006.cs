...
namespace Code.Lavos.Core
{
    ...
    public class LightEngine : MonoBehaviour
    {
        ...
        public static LightEngine Instance
        {
            get
            {
                if (_instance == null)
                {
                    ...
                    if (_instance == null)
                    {
                        Debug.LogWarning("[LightEngine] No LightEngine found in scene! Creating new one.");
                        ...
                        // For persistent lighting across scenes, add LightEngine to a persistent scene
                    }
                    else
                    {
                        Debug.Log("[LightEngine] Found existing instance in scene");
                    }
                }
                return _instance;
            }
        ...
        void Start()
        {
            ...
            Log($"[LightEngine] Start complete");
        }
        ...
        private void InitializeLightPool()
        {
            ...
            _lightRoot.localPosition = Vector3.zero;
        ...
        {
            Debug.Log($"[LightEngine] RegisterLight called - enableDynamicLights={enableDynamicLights}, useBakedLighting={useBakedLighting}");
            ...
            lightData.light.range = (range ?? lightData.baseRange) * 1.5f; // Boost range for URP
            lightData.light.intensity = ((intensity ?? lightData.baseIntensity) * 2f) * globalEmissionMultiplier; // Boost intensity for URP
            ...
            lightData.light.shadows = LightShadows.None; // Disable shadows for performance
        ...
        public void SetLightColor(string sourceId, Color color)
        {
            for (int i = 0; i < _lightPool.Count; i++)
            {
                ...
                if (lightData.isActive && lightData.sourceId == sourceId && lightData.light != null)
                {
                    ...
                    _lightPool[i] = new LightData
                    {
                        light = lightData.light,
                        parent = lightData.parent,
                        offset = lightData.offset,
                        baseIntensity = lightData.baseIntensity,
                        baseRange = lightData.baseRange,
                        baseColor = color,
                        flickerOffset = lightData.flickerOffset,
                        isActive = lightData.isActive,
                        sourceId = lightData.sourceId
                    };
                    return;
                }
            }
