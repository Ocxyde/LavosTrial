        ...
        public static bool SaveGrid(string mazeId, byte[] gridData, int seed)
        {
            ...
            try
            {
                ...
                using (var ms = new MemoryStream())
                {
                    ...
                    ms.Write(encryptedData, 0, (int)encryptedData.Length);
        ...
        public static bool SaveGridWithMetadata(string mazeId, byte[] gridData, byte[] metadata, int seed)
        {
            ...
            try
            {
                using (var ms = new MemoryStream())
                {
                    ...
                    WriteInt(ms, (int)gridData.Length);
                    byte[] encryptedGrid = EncryptData(gridData, seed);
                    ms.Write(encryptedGrid, 0, (int)encryptedGrid.Length);

                    // Write metadata length and data
                    if (metadata != null && metadata.Length > 0)
                    {
                        WriteInt(ms, (int)metadata.Length);
                        byte[] encryptedMetadata = EncryptData(metadata, seed ^ 0xDEADBEEF); // Different XOR key
                        ms.Write(encryptedMetadata, 0, (int)encryptedMetadata.Length);
                    }
                    else
                    {
                        WriteInt(ms, 0);
            ...
            try
            {
                ...
                using (var ms = new MemoryStream(fileData))
                {
                    ...
                    if (metadataLength > 0)
                    {
                        ...
                        ms.Read(encryptedMetadata, 0, (int)metadataLength);
