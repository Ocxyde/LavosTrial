...
namespace Code.Lavos.Core
{
    ...
    public class TestDoubleDoor : MonoBehaviour
    {
        ...
        void CreateDoubleDoors(Transform parent, float holeX, float holeY, float holeZ)
        {
            ...
            // ============================================================
            // 
            // Door layout (top-down view, looking from +Y down):
            //
            ...
            //
            // Meeting point (center of doorway): X = 0
            // Left door: hinges on RIGHT edge, extends LEFT
            // Right door: hinges on LEFT edge, extends RIGHT
            //
            // ============================================================
            ...
            // ============================================================
            // PIVOT POSITIONS FOR EACH MODE
            // ============================================================

            float leftPivotZ, rightPivotZ;
            float leftSwingDir, rightSwingDir; // +1 or -1 for swing direction

            switch (swingMode)
            {
                case SwingMode.BothOutswing:
                    // Both swing OUT (toward viewer, -Z direction)
                    leftPivotZ = wallThickness / 2f + 0.01f;
                    rightPivotZ = wallThickness / 2f + 0.01f;
                    leftSwingDir = -1f;  // Left swings toward -Z
                    rightSwingDir = -1f; // Right swings toward -Z
                    Debug.Log("[TestDoubleDoor] BOTH OUTSWING: Both pivots on OUTSIDE edge (+Z)");
                    break;

                case SwingMode.BothInswing:
                    // Both swing IN (into room, +Z direction)
                    leftPivotZ = -wallThickness / 2f - 0.01f;
                    rightPivotZ = -wallThickness / 2f - 0.01f;
                    leftSwingDir = 1f;   // Left swings toward +Z
                    rightSwingDir = 1f;  // Right swings toward +Z
                    Debug.Log("[TestDoubleDoor] BOTH INSWING: Both pivots on INSIDE edge (-Z)");
                    break;
                ...
                case SwingMode.LeftInRightOut:
                    ...
                    leftPivotZ = -wallThickness / 2f - 0.01f;
                    rightPivotZ = wallThickness / 2f + 0.01f;
                    leftSwingDir = 1f;   // Left swings toward +Z (in)
                    rightSwingDir = -1f; // Right swings toward -Z (out)
                    Debug.Log("[TestDoubleDoor] LEFT IN / RIGHT OUT: Asymmetric");
                    break;
                ...
                case SwingMode.LeftOutRightIn:
                    ...
                    leftPivotZ = wallThickness / 2f + 0.01f;
                    rightPivotZ = -wallThickness / 2f - 0.01f;
                    leftSwingDir = -1f;  // Left swings toward -Z (out)
                    rightSwingDir = 1f;  // Right swings toward +Z (in)
                    Debug.Log("[TestDoubleDoor] LEFT OUT / RIGHT IN: Asymmetric");
                    break;
            ...
            // LEFT DOOR
            // ============================================================
            // Hinges on RIGHT edge (at meeting point)
            // Extends LEFT from pivot
            // When closed: occupies X from -doorWidth to 0

            var leftPivotObj = new GameObject("LeftDoorPivot");
            leftPivotObj.transform.parent = parent;
            leftPivotObj.transform.localPosition = new Vector3(meetingPoint, holeY, leftPivotZ);

            leftDoorObj = CreateDoorPanel(-halfDoorWidth, 0, 0, flipY: true); // Flip to extend left
            leftDoorObj.transform.parent = leftPivotObj.transform;
            leftDoorPivot = leftPivotObj.transform;

            Debug.Log($"[TestDoubleDoor] LEFT door pivot: ({meetingPoint:F2}, 0, {leftPivotZ:F2})");
            Debug.Log($"[TestDoubleDoor] LEFT door panel offset: ({-halfDoorWidth:F2}, 0, 0), extends LEFT");

            // ============================================================
            // RIGHT DOOR
            // ============================================================
            // Hinges on LEFT edge (at meeting point)
            // Extends RIGHT from pivot
            // When closed: occupies X from 0 to +doorWidth

            var rightPivotObj = new GameObject("RightDoorPivot");
            rightPivotObj.transform.parent = parent;
            rightPivotObj.transform.localPosition = new Vector3(meetingPoint, holeY, rightPivotZ);

            rightDoorObj = CreateDoorPanel(0, 0, 0, flipY: false); // Extends right
            rightDoorObj.transform.parent = rightPivotObj.transform;
            rightDoorPivot = rightPivotObj.transform;

            Debug.Log($"[TestDoubleDoor] RIGHT door pivot: ({meetingPoint:F2}, 0, {rightPivotZ:F2})");
            Debug.Log($"[TestDoubleDoor] RIGHT door panel offset: (0, 0, 0), extends RIGHT");
