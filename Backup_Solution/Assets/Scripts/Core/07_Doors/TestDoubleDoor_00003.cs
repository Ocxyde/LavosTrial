...
namespace Code.Lavos.Core
{
    ...
    public class TestDoubleDoor : MonoBehaviour
    {
        ...
        {
            // ============================================================
            // DOUBLE DOOR MATH - REVERSED MAGNETIZATION
            // ============================================================
            // 
            // 1. Door's BACK EDGE (hinge edge) magnetizes to hole's CUTTING EDGE
            // 2. Pivot is placed at the hinge (door's back edge)
            // 3. Door panel extends from its hinge edge
            //
            // When CLOSED:
            //   Left door:  back edge (RIGHT side of door) meets at center
            //   Right door: back edge (LEFT side of door) meets at center
            //   → Doors meet perfectly at center, no gap!
            //
            // ============================================================
            ...
            // ============================================================
            // HOLE CUTTING EDGES (where door edges magnetize when closed)
            // ============================================================
            // The "cutting edge" is where the door sits flush in the hole
            
            float holeCuttingEdge_LeftDoor = meetingPoint;   // Left door's back edge meets here
            float holeCuttingEdge_RightDoor = meetingPoint;  // Right door's back edge meets here

            // ============================================================
            // PIVOT POSITIONS - Based on swing mode (Z axis only)
            // ============================================================
            // X position is ALWAYS at the meeting point (center)
            // Z position changes based on IN/OUT swing

            float pivotX = meetingPoint;  // Both doors hinge at center
            ...
            switch (swingMode)
            {
                case SwingMode.BothOutswing:
                    ...
                    Debug.Log("[TestDoubleDoor] BOTH OUT: Pivots on FRONT face (+Z)");
                    break;
                ...
                case SwingMode.BothInswing:
                    ...
                    leftPivotZ = -wallThickness / 2f - 0.01f;
                    rightPivotZ = -wallThickness / 2f - 0.01f;
                    Debug.Log("[TestDoubleDoor] BOTH IN: Pivots on BACK face (-Z)");
                    break;
            ...
            // LEFT DOOR
            // ============================================================
            // Door's BACK EDGE (right side of panel) magnetizes to hole cutting edge (center)
            // Door extends LEFT from its back edge
            // Pivot is at the back edge (hinge)

            var leftPivotObj = new GameObject("LeftDoorPivot");
            leftPivotObj.transform.parent = parent;
            leftPivotObj.transform.localPosition = new Vector3(pivotX, holeY, leftPivotZ);

            // Left door: back edge (RIGHT side of panel) at pivot, extends LEFT
            // Panel local coords: from X=-halfDoorWidth to X=0 (hinge at X=0)
            leftDoorObj = CreateDoorPanel_FlipY(halfDoorWidth, doorHeight, doorDepth, extendLeft: true);
            leftDoorObj.transform.parent = leftPivotObj.transform;
            leftDoorObj.transform.localPosition = Vector3.zero; // Back edge snaps to pivot
            leftDoorPivot = leftPivotObj.transform;

            Debug.Log($"[TestDoubleDoor] LEFT: pivot=({pivotX:F2}, 0, {leftPivotZ:F2}), back edge magnetized to center, extends LEFT");

            // ============================================================
            // RIGHT DOOR
            // ============================================================
            // Door's BACK EDGE (left side of panel) magnetizes to hole cutting edge (center)
            // Door extends RIGHT from its back edge
            // Pivot is at the back edge (hinge)

            var rightPivotObj = new GameObject("RightDoorPivot");
            rightPivotObj.transform.parent = parent;
            rightPivotObj.transform.localPosition = new Vector3(pivotX, holeY, rightPivotZ);

            // Right door: back edge (LEFT side of panel) at pivot, extends RIGHT
            // Panel local coords: from X=0 to X=+halfDoorWidth (hinge at X=0)
            rightDoorObj = CreateDoorPanel_FlipY(halfDoorWidth, doorHeight, doorDepth, extendLeft: false);
            rightDoorObj.transform.parent = rightPivotObj.transform;
            rightDoorObj.transform.localPosition = Vector3.zero; // Back edge snaps to pivot
            rightDoorPivot = rightPivotObj.transform;

            Debug.Log($"[TestDoubleDoor] RIGHT: pivot=({pivotX:F2}, 0, {rightPivotZ:F2}), back edge magnetized to center, extends RIGHT");
        ...
        void ToggleDoors()
        {
            ...
            // ============================================================
            // SWING ANGLES - Based on pivot position and swing direction
            // ============================================================
            // Left door (extends LEFT from pivot):
            //   -90° = swings OUT (toward -Z, toward viewer)
            //   +90° = swings IN (toward +Z, into room)
            //
            // Right door (extends RIGHT from pivot):
            //   +90° = swings OUT (toward -Z, toward viewer)
            //   -90° = swings IN (toward +Z, into room)
            // ============================================================
            ...
            {
                case SwingMode.BothOutswing:
                    // Both swing OUT: Left=-90°, Right=+90° (mirror)
                    leftTargetAngle = isOpen ? -swingAngle : 0f;
                    rightTargetAngle = isOpen ? swingAngle : 0f;
                    break;

                case SwingMode.BothInswing:
                    // Both swing IN: Left=+90°, Right=-90° (mirror)
                    leftTargetAngle = isOpen ? swingAngle : 0f;
                    rightTargetAngle = isOpen ? -swingAngle : 0f;
                    break;

                case SwingMode.LeftInRightOut:
                    // Left IN (+90°), Right OUT (+90°) - same rotation direction
                    leftTargetAngle = isOpen ? swingAngle : 0f;
                    rightTargetAngle = isOpen ? swingAngle : 0f;
                    break;

                case SwingMode.LeftOutRightIn:
                    // Left OUT (-90°), Right IN (-90°) - same rotation direction
                    leftTargetAngle = isOpen ? -swingAngle : 0f;
                    rightTargetAngle = isOpen ? -swingAngle : 0f;
                    break;
