...
namespace Code.Lavos.Core
{
    ...
    public static class DoorCubeFactory
    {
        ...
        public static GameObject[] CreateDoubleDoor(Vector3 position, Quaternion rotation, Transform parent = null)
        {
            ...
            // RIGHT DOOR: mesh as-is, extends RIGHT, hinge at LEFT (center)
            ...
            // Pivot (hinge) at center, mesh extends right
            rightDoor.transform.localPosition = Vector3.zero;

            // LEFT DOOR: rotate 180° Y so it extends LEFT, hinge at RIGHT (center)
            var leftDoor = CreateDoorCube(
                Vector3.zero,
                Quaternion.Euler(0, 180, 0),  // Rotate 180° around Y
                isLeftHanded: true,
                parent: doubleDoorObj.transform
            );
            leftDoor.name = "LeftDoorPanel";
            // After 180° rotation, the hinge (left edge) is now on the RIGHT
            // Position so hinge is at center
            leftDoor.transform.localPosition = Vector3.zero;

            Debug.Log($"[DoorCubeFactory] Double door created (both hinges at CENTER)");
            Debug.Log($"  Right: pos={rightDoor.transform.localPosition}, extends RIGHT");
            Debug.Log($"  Left: pos={leftDoor.transform.localPosition}, rot=180°, extends LEFT");
    ...
    /// Rotates door around Y axis (hinge).
    /// </summary>
    public class DoorAnimator : MonoBehaviour
    {
        ...
        /// <summary>
        /// Animate to an absolute target angle (e.g., 0° closed, 90° open).
        /// </summary>
        public void AnimateToAbsoluteAngle(float angle, float dur)
        {
            startAngle = transform.localEulerAngles.y;
            // Handle angle wrapping (e.g., 350° → 10° should go forward, not backward)
            targetAngle = angle;
            
            // Normalize angles to -180 to 180 range for smooth interpolation
            while (startAngle > 180f) startAngle -= 360f;
            while (startAngle < -180f) startAngle += 360f;
            ...
            Debug.Log($"[DoorAnimator] Absolute: {startAngle:F1}° → {targetAngle:F1}° over {dur}s", this);
        }
