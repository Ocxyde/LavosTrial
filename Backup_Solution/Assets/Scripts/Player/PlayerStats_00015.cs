...
namespace Code.Lavos.Core
{
    ...
    public class PlayerStats : MonoBehaviour
    {
        ...
        {
            // Check if HUDSystem already exists (new system)
            var hudSystem = FindFirstObjectByType<HUDSystem>();
            if (hudSystem != null)
            {
                // HUDSystem exists - no need to create legacy UIBarsSystem
                return;
            }

            // Legacy fallback - create UIBarsSystem only if HUDSystem doesn't exist
            ...
            Debug.LogWarning("[PlayerStats] Created legacy UIBarsSystem - consider using HUDSystem instead");
        }
    }
}
