...
namespace Code.Lavos.Editor
{
    ...
    public class MazeTestSceneSetup : EditorWindow
    {
        ...
        public static void QuickSetup()
        {
            ...
            ConfigureComponent(go, "MazeTorchTest", 21, true, 3.5f, true);

            Selection.activeGameObject = go;
            Debug.Log("[MazeTestSetup] Quick setup complete! Press Play to test.");
        }

        private static void ConfigureComponent(GameObject go, string typeName, int mazeSize = 21, bool includeTorches = true, float cameraDistance = 3.5f, bool spawnPlayer = true)
        {
            var type = System.Type.GetType($"Code.Lavos.Core.{typeName}, Code.Lavos.Core");
            if (type == null) return;
