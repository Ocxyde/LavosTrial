...
namespace Code.Lavos.Editor
{
    ...
    public class SceneSetupHelper : EditorWindow
    {
        ...
        {
            // Create HUDSystem with Canvas on same GameObject (required for bars to work)
            if (!CheckHUDSystem())
            {
                var hudGO = new GameObject("HUDSystem");
                
                // Add Canvas FIRST (HUDSystem needs it)
                ...
                // Add HUDSystem (will auto-construct bars in Awake)
                hudGO.AddComponent<HUDSystem>();
                
                Debug.Log("✅ Added HUDSystem with Canvas (Complete dynamic HUD)");
            }
            else
            {
            ...
            // Canvas must be on the same GameObject as HUDSystem for bars to work
        }
