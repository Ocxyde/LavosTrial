...
namespace Code.Lavos.Core
{
    ...
    public class FpsMazeTest : MonoBehaviour
    {
        ...
        {
            // Find and remove any old ground objects (quad/plane)
            var oldGround = GameObject.Find("GroundPlane");
            if (oldGround != null)
            {
                Debug.Log("[FpsMazeTest] 🗑️ Removing old ground...");
                DestroyImmediate(oldGround);
            }
            
            // Find and remove any old ceiling objects
            var oldCeiling = GameObject.Find("CeilingPlane");
            if (oldCeiling != null)
            {
                Debug.Log("[FpsMazeTest] 🗑️ Removing old ceiling...");
                DestroyImmediate(oldCeiling);
            }
            
            // Create new flat cube ground with pixel art stone texture
            var ground = GroundPlaneGenerator.CreateGroundCube(groundSize, 32);
            Debug.Log($"[FpsMazeTest] ✅ Ground cube created ({groundSize}x{groundSize}m, flat cube with stone texture)");
            
            // Create ceiling with dark stone texture
            var ceiling = CeilingGenerator.CreateCeilingCube(groundSize, 32);
            Debug.Log($"[FpsMazeTest] ✅ Ceiling cube created ({groundSize}x{groundSize}m, dark stone texture)");
        }
