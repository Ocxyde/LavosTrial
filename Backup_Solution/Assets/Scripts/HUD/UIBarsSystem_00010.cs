    ...
    public class UIBarsSystem : MonoBehaviour
    {
        ...
        void SetInitialValues()
        {
            ...
            if (playerStatsInstance != null)
            {
                ...
                SetHealth(
                    currentHealthProp?.GetValue(playerStatsInstance) is float ch ? ch : 1000f,
                    maxHealthProp?.GetValue(playerStatsInstance) is float mh ? mh : 1000f
                );
                SetMana(
                    currentManaProp?.GetValue(playerStatsInstance) is float cm ? cm : 500f,
                    maxManaProp?.GetValue(playerStatsInstance) is float mm ? mm : 500f
                );
                SetStamina(
