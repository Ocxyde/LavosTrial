...
//
// USAGE:
//   Export: ShareSystem.ExportMaze()
//   Import: ShareSystem.ImportMaze(code)
//   Share: ShareSystem.ShareMaze()
//   Custom: ShareSystem.RegisterExporter/Importer()
//
...
{
    /// <summary>
    /// ShareSystem - Tactical maze sharing system with extensible pattern.
    ...
    public static class ShareSystem
    {
        ...
        public static string ExportMaze()
        {
            ...
            if (builder == null)
            {
                Debug.LogWarning("[ShareSystem] CompleteMazeBuilder8 not found in scene");
                return string.Empty;
            }
            ...
            Debug.Log($"[ShareSystem] Exported maze code: {code}");
        ...
        public static bool ImportMaze(string code)
        {
            ...
            if (ImportCode(code, out uint seed, out int level))
            {
                ...
                if (builder != null)
                {
                    Debug.Log($"[ShareSystem] Ready to generate: Seed={seed}, Level={level}");
                    return true;
                }
                else
                {
                    Debug.LogWarning("[ShareSystem] CompleteMazeBuilder8 not found - cannot regenerate");
                }
            }
        ...
        public static bool ImportCode(string code, out uint seed, out int level)
        {
            ...
            {
                Debug.LogWarning("[ShareSystem] Import failed: code is empty");
                return false;
            }
            ...
            if (parts.Length != 4)
            {
                Debug.LogWarning($"[ShareSystem] Import failed: invalid format (expected 4 parts, got {parts.Length})");
                return false;
            }
            ...
            if (parts[0] != PREFIX)
            {
                Debug.LogWarning($"[ShareSystem] Import failed: invalid prefix '{parts[0]}' (expected '{PREFIX}')");
                return false;
            }
            ...
            {
                Debug.LogWarning($"[ShareSystem] Import failed: invalid seed '{parts[1]}'");
                return false;
            }
            ...
            {
                Debug.LogWarning($"[ShareSystem] Import failed: invalid level '{parts[2]}'");
                return false;
            }
            ...
            {
                Debug.LogWarning($"[ShareSystem] Import failed: checksum mismatch (expected {expectedChecksum}, got {actualChecksum})");
                return false;
            }

            Debug.Log($"[ShareSystem] Imported code: Seed={seed}, Level={level}");
            return true;
        }
        ...
        public static void CopyToClipboard(string code)
        {
            ...
            try
            {
                ...
                Debug.Log($"[ShareSystem] Copied to clipboard: {code}");
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[ShareSystem] Clipboard copy failed: {ex.Message}");
                Debug.Log($"[ShareSystem] Manual copy: {code}");
            }
        }
        ...
        public static void RegisterExporter(Func<uint, int, string> exporter)
        {
            ...
            Debug.Log("[ShareSystem] Custom exporter registered");
        }
        ...
        public static void RegisterImporter(Func<string, bool> importer)
        {
            ...
            Debug.Log("[ShareSystem] Custom importer registered");
        }
        ...
        public static void ClearExporter()
        {
            ...
            Debug.Log("[ShareSystem] Custom exporter cleared");
        }
        ...
        public static void ClearImporter()
        {
            ...
            Debug.Log("[ShareSystem] Custom importer cleared");
        }
    ...
    public static class CompleteMazeBuilder8SharingExtensions
    {
        ...
        {
            return ShareSystem.ImportMaze(code);
        }
        ...
        {
            string code = ShareSystem.ExportMaze();
