...
namespace Code.Lavos.Geometry
{
    ...
    public struct Tetrahedron
    {
        ...
        public double SurfaceArea()
        {
            ...
            double area1 = Vector3d.Cross(ab, ac).Magnitude() / 2.0; // ABC
            double area2 = Vector3d.Cross(ab, ad).Magnitude() / 2.0; // ABD
            double area3 = Vector3d.Cross(ac, ad).Magnitude() / 2.0; // ACD
            double area4 = Vector3d.Cross(bc, bd).Magnitude() / 2.0; // BCD
        ...
        {
            // Compute volumes of sub-tetrahedrons
            Tetrahedron pBCD = new Tetrahedron(point, B, C, D);
            Tetrahedron APCD = new Tetrahedron(A, point, C, D);
            Tetrahedron ABPD = new Tetrahedron(A, B, point, D);
            Tetrahedron ABCP = new Tetrahedron(A, B, C, point);
            
            double totalVolume = Volume();
            double subVolume = pBCD.Volume() + APCD.Volume() + ABPD.Volume() + ABCP.Volume();
            
            // Point is inside if sum of sub-volumes equals total volume (with epsilon)
            const double epsilon = 1e-10;
            return Math.Abs(subVolume - totalVolume) < epsilon;
        }
