...
namespace Code.Lavos.Editor
{
    ...
    public class MazeTestSceneSetup : EditorWindow
    {
        ...
        private void AddAllComponents(GameObject go)
        {
            ...
            EnsureComponentByName(go, "SpatialPlacer");
            EnsureComponentByName(go, "MazeIntegration");
            EnsureComponentByName(go, "TestMazeWithTorches");
            ...
            // Configure TestMazeWithTorches
            var testScript = GetComponentByName(go, "TestMazeWithTorches");
            if (testScript != null)
            {
                var widthField = testScript.GetType().GetField("mazeWidth");
                var heightField = testScript.GetType().GetField("mazeHeight");
                var torchesField = testScript.GetType().GetField("placeTorches");
        ...
        public static void QuickSetup()
        {
            ...
            // Add all required components using string names
            go.AddComponent(typeof(MonoBehaviour)); // Placeholder, will be replaced
            
            // Add components by type name
            string[] componentTypes = new string[]
            {
                "MazeGenerator", "MazeRenderer", "TorchPool", "SpatialPlacer",
                "MazeIntegration", "TestMazeWithTorches", "RoomGenerator",
            ...
            ConfigureComponent(go, "TestMazeWithTorches", 21, true);

            Selection.activeGameObject = go;
            Debug.Log("[MazeTestSetup] Quick setup complete! Press Play to test.");
        }

        private static void ConfigureComponent(GameObject go, string typeName, int mazeSize = 21, bool includeTorches = true)
        {
            var type = System.Type.GetType($"Code.Lavos.Core.{typeName}, Code.Lavos.Core");
            if (type == null) return;
            ...
            else if (typeName == "TestMazeWithTorches")
            {
                var widthField = type.GetField("mazeWidth");
                var heightField = type.GetField("mazeHeight");
                var placeTorchesField = type.GetField("placeTorches");
